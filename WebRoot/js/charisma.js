...
function docReady(){
    ...
    //-----------------------------------------------------------------------------
    //生成一个 会员月消费记录的曲线图    数据源
    var monthdata = [], monthtotalPoints = 12;
    function getMonthRandomData() {
		if (monthdata.length > 0)
			monthdata = monthdata.slice(1);
	...
	if($("#sincos").length)
	{
		...
		for (var i = 1; i < 13; i += 0.5) {
		    //去掉一个曲线
		    sin = getMonthRandomData();
			//sin.push([i, Math.sin(i)/i]);
			//cos.push([i, Math.cos(i)]);
		}

		//------------
		//右上角 曲线 说明，这里是两个点的曲线说明
		/*
	 ...
	 */
	 ...
	 var options = {
		...
		//绘图的函数 
	 var plot = $.plot($("#sincos"),[ { data: sin, label: "在线会员月消费"}], options);
	 //*************************************************************************************
		...
		$("#sincos").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("你点击了"+ item.series.label +"中的" + item.dataIndex + "月份数据.");
	...
	var donutdata = [
	{ label: "小于20岁",  data: 12},
	{ label: "20至30岁",  data: 27},
	{ label: "30至40岁",  data: 85},
	{ label: "大于40岁",  data: 64}
   ];
